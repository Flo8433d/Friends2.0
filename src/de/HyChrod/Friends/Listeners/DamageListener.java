				...
				if(p.getItemInHand() != null) {
					if(p.getItemInHand().hasItemMeta()) {
						if(p.getItemInHand().getItemMeta().hasDisplayName()) {
							if(p.getItemInHand().getItemMeta().getDisplayName().equals(ItemStacks.FRIENDITEM(p).getItemMeta().getDisplayName())) {
								e.setCancelled(true);
								...
								ReflectionsManager.sendHoverMessage(toAdd, p.getName(), plugin.getString("Messages.Commands.Add.Add.ToAdd.2")
