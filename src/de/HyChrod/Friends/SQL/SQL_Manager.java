...
public class SQL_Manager {
	...
	public static Boolean playerExists(OfflinePlayer player) {
		try {
			ResultSet rs = MySQL.query("SELECT * FROM friends2_0 WHERE UUID= '" + player.getUniqueId().toString() + "'");
	...
	public static Boolean createPlayer(OfflinePlayer player) {
		if(!(playerExists(player))) {
			MySQL.update("INSERT INTO friends2_0(UUID, FRIENDS, BLOCKED, REQUESTS, OPTIONS, LASTONLINE) VALUES ('" + player.getUniqueId().toString() + "', '', '', '','','');");
	...
	public static Boolean setLastOnline(OfflinePlayer player, Long timeStamp) {
		if(playerExists(player)) {
			MySQL.update("UPDATE friends2_0 SET LASTONLINE='" + timeStamp.toString() + "' WHERE UUID='" + player.getUniqueId().toString() + "';");
			return true;
		} else {
			createPlayer(player);
	...
	public static void setBlocked(List<OfflinePlayer> currentBlocked, OfflinePlayer player) {
		if(playerExists(player)) {
			...
			MySQL.update("UPDATE friends2_0 SET BLOCKED='" + serializedBlocked + "' WHERE UUID='" + player.getUniqueId().toString() + "';");
	...
	public static void setRequests(List<OfflinePlayer> currentRequests, OfflinePlayer player) {
		if(playerExists(player)) {
			...
			MySQL.update("UPDATE friends2_0 SET REQUESTS='" + serializedRequests + "' WHERE UUID='" + player.getUniqueId().toString() + "';");
	...
	public static void setFriends(List<OfflinePlayer> currentFriends, OfflinePlayer player) {
		if(playerExists(player)) {
			...
			MySQL.update("UPDATE friends2_0 SET FRIENDS='" + serializedFriends + "' WHERE UUID='" + player.getUniqueId().toString() + "';");
	...
	public static void setOptions(OfflinePlayer player, List<String> options) {
		if(playerExists(player)) {
			...
			MySQL.update("UPDATE friends2_0 SET OPTIONS='" + serializedOptions + "' WHERE UUID='" + player.getUniqueId().toString() + "';");
		} else {
			createPlayer(player);
