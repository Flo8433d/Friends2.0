...
public class PlayerUtilities {
	...
	public PlayerUtilities(OfflinePlayer player) {
		...
		if(MySQL.isConnected()) this.sql = true;
		if(!PlayerUtilities.friends.containsKey(player) && !PlayerUtilities.requests.containsKey(player) && !PlayerUtilities.blocked.containsKey(player)) {
			this.loadData();
		}
	}
	...
	public static void fullSave() {
		...
		for(OfflinePlayer player : requests.keySet()) {
			new PlayerUtilities(player).saveData();
		}
		for(OfflinePlayer player : blocked.keySet()) {
			new PlayerUtilities(player).saveData();
		}
	}
	...
	public LinkedList<OfflinePlayer> getRequests() {
		...
		if(PlayerUtilities.requests.containsKey(this.player)) currentRequests = PlayerUtilities.requests.get(this.player);
	...
	public LinkedList<String> getOptions() {
		...
		if(PlayerUtilities.options.containsKey(this.player)) currentOptions = PlayerUtilities.options.get(this.player);
	...
	public void saveData() {
		...
		if(PlayerUtilities.requests.containsKey(this.player)) {
			...
			this.mgr.save(file, cfg, "Players." + this.player.getUniqueId().toString() + ".Requests", serializedRequests);
		}
		...
		if(PlayerUtilities.options.containsKey(this.player)) {
			this.mgr.save(file, cfg, "Players." + this.player.getUniqueId().toString() + ".Options", PlayerUtilities.options.get(this.player));
		}
	}
	...
	public void loadData() {
		...
		if(this.cfg.getString("Players." + this.player.getUniqueId().toString() + ".Requests") != null) {
			LinkedList<OfflinePlayer> currentRequests = new LinkedList<>();
			if(PlayerUtilities.requests.containsKey(this.player)) currentRequests = PlayerUtilities.requests.get(this.player);
			for(String uuid : this.cfg.getStringList("Players." + this.player.getUniqueId().toString() + ".Requests")) {
		...
		if(this.cfg.getString("Players." + this.player.getUniqueId().toString() + ".Options") != null) {
			LinkedList<String> currentOptions = new LinkedList<>();
			if(PlayerUtilities.options.containsKey(this.player)) currentOptions = PlayerUtilities.options.get(this.player);
			for(String option : this.cfg.getStringList("Players." + this.player.getUniqueId().toString() + ".Options")) {
				if(!currentOptions.contains(option)) {
					currentOptions.add(option);
				}
			}
