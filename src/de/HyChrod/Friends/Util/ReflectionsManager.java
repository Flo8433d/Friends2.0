...
public class ReflectionsManager {
	...
	public static void sendHoverMessage(Player player, Player adder, String message, String[] msgs, String[] hover, String[] command) {
		try {
			
			String format = "{\"text\":\"" + message + "\",\"extra\":[";
			for(int i = 0; i < msgs.length; i++) {
				format = format + "{\"text\":\"" + msgs[i] + "   " + "\",\"hoverEvent\":{\"action\":\"show_text\", \"value\":\""
						+ "" + hover[i] + "\"},\"clickEvent\":{\"action\":\"run_command\",\"value\":\"" + command[i].replace("%name%", adder.getName()) + "\"}},";
			}
